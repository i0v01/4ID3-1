...
try:
    ...
    if connection.is_connected():
        ...
        cursor.close() 
        print("    > Dropped table {DEVICE_ID}")
        cursor = connection.cursor()
        cursor.execute(f"""CREATE TABLE IF NOT EXISTS `{GROUP_NAME}`.`{DEVICE_ID}`
                    (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `DeviceName` VARCHAR(45) NOT NULL,
...
def main():
    ...
    try:
        ...
        for x in sman.get().keys():
            if(x["Type"] == "DIO"):
                remote_device.set_io_configuration(x["IO"], IOMode.DIGITAL_IN)
                remote_device.set_dio_change_detection({x["IO"]})
            
            elif(x["Type"] == "AIO"):
                remote_device.set_io_configuration(x["IO"], IOMode.ADC)
            ...
            for x in data.keys():
                ...
                try:
                    connection = mysql.connector.connect(host=HOST_IP,
                                            ...
                                            database=DB_NAME)
                    cursor = connection.cursor()
                    cursor.execute(f"""INSERT INTO `{TABLE_NAME}` (`Time`, `Sensor`, `Value`) VALUES ('{str(now.strftime('%d/%m/%Y %H:%M:%S'))}', '{str(x)}', '{str(data[x]["Curr"])}'); """)
                    cursor.close()
                    connection.commit()
                    connection.close()
                except:
