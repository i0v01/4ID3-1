        ...
        if self.path.endswith(''):
            ...
            out += """
                <form method="POST" action="/data">
                    ...
                    <input type="text" id="dbname" name="dbname" value = "iot"></input>
                    <br />
                    <br />
                    <label for="dbtable">Database Table:</label>
                    <input type="text" id="dbtable" name="dbtable" value = "data"></input>
                    <br />
                    <br />
                    <label for="filters">Choose a Filter:</label>
                    <select name="filters" id="filters">
                        <option value="iddata">iddata</option>
    ...
    def do_POST(self):
        ...
        if self.path.endswith('/data'):
            ...
            try:
                ...
                print('cursor')
                query = str(f"SELECT * FROM `{ifExists('dbname')}`.`{ifExists('dbtable')}` WHERE `{ifExists('dbtable')}`.`{ifExists('filters')}` {ifExists('comparison')} {ifExists('val')} ORDER BY `iddata` {ifExists('order')};") # ORDER BY `{ifExists('tbtable')}`.`iddata` {ifExists('order')} ; ")
            ...
            self.wfile.write(f"""
                    ...
                    <br />
                        ...
                        <br />
                        <br />
                        <b>FORM DEBUG:</b> {str(postVals)} <br />
                        <b>QUERY DEBUG:</b> {str(query)} <br /> <br />
                        <br/>
                        {out}
                        <br />
